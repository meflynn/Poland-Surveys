...
model_3_full_response_f <- function(modeldata, priordata) {
  ...
  PRIORS <- get_prior(bf(troops_100km_full ~ 0 + Intercept + treatment_group+ gender + age + income + income_source + minority + education + ideology_z + (1 |ID1| province),
    ...
    # Need this second case_when for some reason. Putting it in one didn't work.
    prior = case_when(
      class == "cor" ~ "lkj(1)",
      TRUE ~ prior
    ))
...
model_5_ordered_response_f <- function(modeldata, priordata) {
  ...
  predictors_formula <- "treatment_group + gender + age + income + income_source + minority + education + ideology_z + (1 |ID1| province)"
