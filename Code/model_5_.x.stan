...
functions {
  ...
  real partial_log_lik_lpmf(array[] int seq, int start, int end,
                            data array[] int Y, data int nthres, vector bQ,
                            data matrix XQ, vector Intercept, real disc,
...
model {
  ...
  if (!prior_only) {
    target += reduce_sum(partial_log_lik_lpmf, seq, grainsize, Y, nthres, bQ,
                         XQ, Intercept, disc, J_1, Z_1_1, r_1_1);
  }
